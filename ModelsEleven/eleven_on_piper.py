...
#
# this particular dataset has problems with HMMs, so we modify data a bit
#
dynamic_train[:, 10, :] = dynamic_train[:, 10, :] + np.random.uniform(-1e-7, 1e-7, dynamic_train[:, 10, :].shape)
dynamic_val[:, 10, :] = dynamic_val[:, 10, :] + np.random.uniform(-1e-7, 1e-7, dynamic_val[:, 10, :].shape)

dynamic_train[:, [10], :] = dynamic_train[:, [10], :]                   / 1000000.0
dynamic_train[:, [3,7,9,13], :] = dynamic_train[:, [3,7,9,13], :]       / 10000000.0
dynamic_train[:, [4,6,8,12,14], :] = dynamic_train[:, [4,6,8,12,14], :] / 100000000.0
dynamic_train[:, [5,11], :] = dynamic_train[:, [5,11], :]               / 1000000000.0
dynamic_val[:, [10], :] = dynamic_val[:, [10], :]                   / 1000000.0
dynamic_val[:, [3,7,9,13], :] = dynamic_val[:, [3,7,9,13], :]       / 10000000.0
dynamic_val[:, [4,6,8,12,14], :] = dynamic_val[:, [4,6,8,12,14], :] / 100000000.0
dynamic_val[:, [5,11], :] = dynamic_val[:, [5,11], :]               / 1000000000.0

static_train[:, [3,5]] = static_train[:, [3,5]]     / 100000000.0
static_train[:, [2,4,6]] = static_train[:, [2,4,6]] / 1000000000.0
static_val[:, [3,5]] = static_val[:, [3,5]]     / 100000000.0
static_val[:, [2,4,6]] = static_val[:, [2,4,6]] / 1000000000.0
